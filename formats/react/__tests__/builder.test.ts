...
const templateInput = (): ReactSignatureTemplateInput => ({
  id: "template-1",
...
const templateWithOutOfBoundsField = (): ReactSignatureTemplate => ({
  id: "template-1",
  name: "Onboarding agreement",
  documents: [
    {
...
describe("React signature builder", () => {
  ...
  it.effect("creates a validated builder state from template input", () =>
    Effect.gen(function* () {
      const state = yield* createReactSignatureBuilderState({
        template: {
          ...templateInput(),
          fields: [
            {
  ...
  it.effect("validates browser A1 PDF signing options with timestamping", () =>
    Effect.gen(function* () {
      ...
      const withField = yield* addReactSignatureField(template, field);
      const decoded = yield* Schema.decodeUnknownEffect(BrowserPdfSigningInputSchema)({
        pdf,
        template: withField,
        fieldId: "signature-1",
  ...
  it.effect("centers and clamps pointer placement against the target page", () =>
    Effect.gen(function* () {
      const template = yield* createReactSignatureTemplate(templateInput());
      const placed = yield* placeReactSignatureField(template, {
        documentId: "document-1",
        pageIndex: 0,
        x: 5,
        y: 5,
  ...
  it.effect("rejects fields outside the declared page", () =>
    Effect.gen(function* () {
      const result = yield* Effect.result(
        validateReactSignatureTemplate(templateWithOutOfBoundsField()),
      );

      expect(Result.isFailure(result)).toBe(true);
      if (Result.isFailure(result)) {
        expect(result.failure.code).toBe("react.FIELD_OUT_OF_BOUNDS");
      }
    }),
  );
  ...
  it.effect("loads uploaded browser PDFs into builder page geometry", () =>
    Effect.gen(function* () {
      const pdf = yield* createPdf;
      const blobBuffer = new ArrayBuffer(pdf.byteLength);
      new Uint8Array(blobBuffer).set(pdf);
      const bytes = yield* readBrowserFileBytes(
        new Blob([blobBuffer], { type: "application/pdf" }),
      );
      const document = yield* loadBrowserPdfDocument({
        id: "uploaded",
        name: "uploaded.pdf",
        pdf: bytes,
      });
      ...
      const state = yield* createBrowserPdfSignatureBuilderState({
        ...
        name: "Browser PDF builder state",
        documentId: "uploaded",
        documentName: "uploaded.pdf",
        pdf,
        role: { id: "signer-1", label: "Cliente", email: "ana@example.com", required: true },
      ...
      const explicitDocument = yield* loadBrowserPdfDocument({
  ...
  it.effect("auto-places browser PDF builder signatures", () =>
    Effect.gen(function* () {
      const pdf = yield* createPdf;
      const state = yield* createBrowserPdfSignatureBuilderState({
        id: "browser-auto-builder-state",
        name: "Browser PDF auto builder state",
        documentId: "uploaded",
        documentName: "uploaded.pdf",
        pdf,
        role: { id: "signer-1", label: "Cliente", email: "ana@example.com", required: true },
      ...
      const ambiguous = yield* Effect.result(
        createBrowserPdfSignatureBuilderState({
          ...
          name: "Browser PDF ambiguous builder state",
          documentId: "uploaded",
          documentName: "uploaded.pdf",
          pdf,
          role: { id: "signer-1", label: "Cliente", email: "ana@example.com", required: true },
  ...
  it.effect("signs a batch of PDFs sequentially and captures per-item failures", () =>
    Effect.gen(function* () {
      ...
      const results = yield* signBrowserPdfBatch(
        [
          { id: "a.pdf", input },
          { id: "b.pdf", input },
        ],
      ...
      for (const result of results) {
        if (!result.ok) {
          expect(result.error.code).toBe(ReactIntegrationErrorCodeValue.unknownField);
        }
      }
    }),
  );
