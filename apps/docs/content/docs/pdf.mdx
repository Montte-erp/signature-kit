---
title: PDF / PAdES
description: "Sign PDFs with PAdES via signPdf, apply the ICP-Brasil policy (AD-RB), and verify with verifyPdf — everything about the Signatures seam."
---

**What you'll do:** sign PDF bytes in the PAdES standard with `signPdf`, choose between the `"pades-ades"` and `"pades-icp-brasil"` (AD-RB) policies, and check the result with `verifyPdf`. The signing power comes from the `Signatures` seam, provided by `a1SignaturesLayer` — the PDF module only handles mutating the document.
...
`signatureLength` reserves the space (in bytes) of the `/Contents` field that holds the CMS — a value that is too short makes the signature fail. Without `reason` and `name`, the defaults are `"Digital signature"` and `"SignatureKit signer"`. The remaining optional fields include `contactInfo`, `location`, `signingTime`, `timestamp`, and `appearance`.
...
`appearance.placement.kind: "auto"` computes a visible rectangle on the chosen page, starts from the preferred anchor, and avoids collision with existing widgets/annotations. This replaces manually guessing coordinates when you just want a signature in the available footer/corner. Without `appearance`, the signature stays invisible for compatibility.
...
The explicit object carries `policyOid`, `policyHash`, `policyHashAlgorithm`, and `policyUri` — the same identifiers of the AD-RB SP that the automatic fetch would bring, now pinned in your code.
...
`byteRange` is the `[number, number, number, number]` quadruple covered by the signature, and `signerSerialNumber` is the serial of the signer certificate or `null` when there is no signature.
...
<Cards>
  ...
  <Card title="Remote signers" href="/docs/providers">Provider packages for DocuSign, Clicksign, Assinafy, ZapSign, DocuSeal, Adobe Acrobat Sign, Dropbox Sign, and Documenso.</Card>
