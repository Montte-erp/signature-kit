---
title: PDF / PAdES
description: "Assine PDFs com PAdES via signPdf, aplique a política ICP-Brasil (AD-RB) e verifique com verifyPdf — tudo sobre a camada Signatures."
---

**O que você vai fazer:** assinar os bytes de um PDF no padrão PAdES com `signPdf`, escolher entre as políticas `"pades-ades"` e `"pades-icp-brasil"` (AD-RB) e conferir o resultado com `verifyPdf`. O poder de assinar vem da camada `Signatures`, fornecida pelo `a1SignaturesLayer` — o módulo de PDF cuida apenas de modificar o documento.
...
`signatureLength` reserva o espaço (em bytes) do campo `/Contents` que guarda o CMS — um valor curto demais faz a assinatura falhar. Sem `reason` e `name`, os padrões são `"Digital signature"` e `"SignatureKit signer"`. Os demais campos opcionais incluem `contactInfo`, `location`, `signingTime`, `timestamp` e `appearance`.
...
`appearance.placement.kind: "auto"` calcula um retângulo visível na página escolhida, parte da âncora preferida e evita colisão com widgets/anotações existentes. Isso substitui o trabalho de adivinhar coordenadas manualmente quando você só quer uma assinatura no rodapé/canto disponível. Sem `appearance`, a assinatura permanece invisível por compatibilidade.
...
O objeto explícito carrega `policyOid`, `policyHash`, `policyHashAlgorithm` e `policyUri` — os mesmos identificadores da SP AD-RB que a busca automática traria, agora fixados no seu código.
...
`byteRange` é a quádrupla `[number, number, number, number]` coberta pela assinatura, e `signerSerialNumber` é o serial do certificado do signatário ou `null` quando não há assinatura.
...
<Cards>
  ...
  <Card title="Signatários remotos" href="/docs/providers">Pacotes de provedores para DocuSign, Clicksign, Assinafy, ZapSign, DocuSeal, Adobe Acrobat Sign, Dropbox Sign e Documenso.</Card>
