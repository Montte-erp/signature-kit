...
<Cards>
  ...
  <Card title="signature-kit.UNSUPPORTED_ALGORITHM" href="/docs/signing/errors#error-catalog">The loaded PKCS#12 container itself uses an unsupported encryption algorithm, raised while parsing the certificate/key — not for picking a `SignatureAlgorithm` outside the sign catalog, which instead fails as `signature-kit.INVALID_INPUT`.</Card>
