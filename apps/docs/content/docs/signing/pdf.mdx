...
- [`pdf.SIGN_FAILED`](./errors) — unsupported `hashAlgorithm` (only `sha384` is rejected).
- [`pdf.SIGNATURE_TOO_LARGE`](./errors) — the CMS exceeded the reserved space; increase `signatureLength`.
- [`pdf.SIGNATURE_PLACEMENT_FAILED`](./errors) — automatic placement found no space or received invalid dimensions.
- [`pdf.INVALID_PDF`](./errors) — the bytes don't form a valid PDF to sign or verify.
- [`pdf.PLACEHOLDER_NOT_FOUND`](./errors) — `verifyPdf` found no `/ByteRange`, e.g. an unsigned PDF.
- [`signature-kit.WRONG_PASSWORD`](./errors) — wrong A1 password when providing the signatures service.

## Continue from here

<Cards>
  <Card title="XML-DSig" href="./xml">The other format over the same Signatures seam, for NF-e XML.</Card>
  <Card title="Provider APIs" href="../providers/request-shape">Provider packages for Clicksign, Assinafy, ZapSign, DocuSeal, and Documenso.</Card>
  <Card title="Typed error channel" href="./errors">The catalog of pdf.* and signature-kit.* codes that can fail.</Card>
</Cards>
