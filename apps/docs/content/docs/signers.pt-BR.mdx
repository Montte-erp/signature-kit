...
**A ideia central:** o `SignerAdapter` é a costura por onde o poder de assinar entra. Ele responde _de onde_ vem a chave — um `.pfx` A1, um HSM, um serviço remoto — enquanto tudo acima dele (core, XML, PDF) conversa com um único formato fixo, não com o backend.
...
- `id: string` — identidade do backend de assinatura. O A1 usa `"a1"`. Provedores remotos como Clicksign, Assinafy, ZapSign, DocuSeal e Documenso usam funções `create*SignatureRequest(...)` e não passam por esta costura de mutação de XML/PDF.
