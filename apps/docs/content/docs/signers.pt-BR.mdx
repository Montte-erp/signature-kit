...
description: "O SignerAdapter é a costura de capacidade: ele decide de onde vem o poder de assinar sem tocar no documento, é disponibilizado como o serviço Signatures e pode ser trocado sem reescrever os formatos."
---

**A ideia central:** o `SignerAdapter` é a única costura por onde o poder de assinar entra. Ele responde _de onde_ vem a chave — um `.pfx` A1, um HSM, um serviço remoto — e sempre expõe o mesmo formato. Tudo acima dele (core, XML, PDF) conversa com esse formato, não com o backend.
...
O tipo abaixo é a definição literal exportada por `@signature-kit/core/config`. O doc-comment fixa a regra que o restante desta página detalha: o signatário decide de onde vem o poder de assinar e **nunca** é dono da mutação de formato.
...
- `id: string` — identidade do backend de assinatura. O A1 usa `"a1"`. Signatários remotos como DocuSign, Clicksign, Assinafy e ZapSign usam funções `create*SignatureRequest(...)` e não passam por esta costura de mutação de XML/PDF.
...
O pacote `@signature-kit/a1` implementa a costura a partir de um arquivo `.pfx`/`.p12`. `loadA1SignerAdapter(options)` faz o parsing assíncrono e retorna um `SignerAdapter` pronto, com `id` igual a `"a1"`.
...
O core define `Signatures` como um `Context.Service` com a tag `"@signature-kit/core/Signatures"`. Você nunca implementa esse serviço na mão: você passa um `SignerAdapter` para `signaturesLayer(signer)` e recebe de volta um `Layer<Signatures>` (via `Layer.succeed`). Os accessors `signatures.{inspect, certificate, importSigningKey, sign, verify}` declaram a dependência de `Signatures` e a satisfazem com a layer fornecida.
...
Para os formatos, o A1 oferece um atalho: `a1SignaturesLayer(options)` retorna um `Layer<Signatures, SignatureKitError>` diretamente, sem que você precise instanciar o adapter antes. `signXml` e `signPdf` exigem `Signatures` no canal de requisitos; basta você entregar essa layer via `Effect.provide`.
...
Como a costura é uma porta de formato fixo, trocar de onde vem o poder de assinar é trocar o argumento de `signaturesLayer(...)`. O trabalho de assinatura é idêntico nos dois casos; nada do formato muda.
...
A regra do doc-comment é deliberada: o `SignerAdapter` é dono de **onde** vem o poder de assinar e **nunca** da mutação do documento. Ele opera sobre `content: Uint8Array` e retorna uma `signature: Uint8Array` — bytes opacos. Embutir uma assinatura em um XML envelopado ou em um PDF PAdES é trabalho dos módulos de formato (`@signature-kit/xml`, `@signature-kit/pdf`), que consomem a costura através de `Effect.provide` e só por isso permanecem neutros em relação ao backend.

Esse corte é o que mantém a aplicação estável: o XML não sabe se a chave veio de um `.pfx` ou de um HSM, e o signatário não sabe se os bytes que recebeu são uma NF-e ou um PDF. Cada lado conhece apenas a sua metade da fronteira.

## Erros que você pode encontrar

Toda falha da costura é um `SignatureKitError` tipado. Os mais comuns ao construir e usar o signatário:

<Cards>
  <Card title="signature-kit.WRONG_PASSWORD" href="/docs/errors#error-catalog">Senha do certificado incorreta em loadA1SignerAdapter.</Card>
  <Card title="signature-kit.NO_PRIVATE_KEY" href="/docs/errors#error-catalog">O arquivo não contém chave privada, então importSigningKey e sign não têm com o que assinar.</Card>
  <Card title="signature-kit.UNSUPPORTED_ALGORITHM" href="/docs/errors#error-catalog">Algoritmo fora de "rsa-sha256" / "rsa-sha512".</Card>
  <Card title="signature-kit.KEY_IMPORT_FAILED" href="/docs/errors#error-catalog">A CryptoKey não pôde ser materializada em importSigningKey.</Card>
  <Card title="signature-kit.SIGN_FAILED" href="/docs/errors#error-catalog">A operação sign falhou no backend.</Card>
</Cards>
