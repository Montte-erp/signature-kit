...
Every failure in the core, the A1 signer, and the formats is a `SignatureKitError` in `Effect`'s error channel — not a thrown exception. The return type carries the failure: `Effect<SignatureArtifact, SignatureKitError>`. This makes handling exhaustive and compiler-verifiable: you discriminate by `code`, and whatever you do not handle keeps propagating, typed.
...
When you only want to intercept a specific failure and let the rest propagate, use `Effect.catchIf` with a predicate over `_tag` and `code`:
...
const request = createDocuSignSignatureRequest(options, {
  title: "Contract",
  documents,
  recipients,
}).pipe(
...
```package-install
@signature-kit/xml @signature-kit/pdf @signature-kit/core @signature-kit/docusign
```
