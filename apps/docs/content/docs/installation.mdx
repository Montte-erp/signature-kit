...
<Steps>
  ...
  <Step title="Add XML and PDF">
    PDF consumes the same boundary via `Effect.provide(layer)`. XML also declares the `xmlRuntimeLayer`, because installing the XML runtime is an explicit dependency.
...
- [`signature-kit.WRONG_PASSWORD`](/docs/errors) — "Wrong certificate password."
- [`signature-kit.NO_PRIVATE_KEY`](/docs/errors) — "The file does not contain a private key."
- [`signature-kit.INVALID_FORMAT`](/docs/errors) — the file is not a PKCS#12 (.pfx/.p12).

## Next steps

<Cards>
  <Card title="First use" href="/docs/quickstart">Load an A1 certificate and produce your first signature in minutes.</Card>
  <Card title="The boundary" href="/docs/signers">The SignerAdapter: the seam through which the power to sign enters.</Card>
</Cards>
