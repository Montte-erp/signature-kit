...
<Steps>
  ...
  <Step title="Adicione XML e PDF">
    O PDF consome a mesma camada de assinatura via `Effect.provide(layer)`. O XML também declara o `xmlRuntimeLayer`, porque instalar o runtime de XML é uma dependência explícita.
...
- [`signature-kit.WRONG_PASSWORD`](/docs/errors) — "Senha do certificado incorreta."
- [`signature-kit.NO_PRIVATE_KEY`](/docs/errors) — "O arquivo não contém uma chave privada."
- [`signature-kit.INVALID_FORMAT`](/docs/errors) — o arquivo não é um PKCS#12 (.pfx/.p12).

## Próximos passos

<Cards>
  <Card title="Primeiro uso" href="/docs/quickstart">Carregue um certificado A1 e produza sua primeira assinatura em minutos.</Card>
  <Card title="A camada de assinatura" href="/docs/signers">O SignerAdapter: a junção por onde entra o poder de assinar.</Card>
</Cards>
