...
<Steps>
  <Step title="Read the identity">
    ...
    `signatures.inspect()` returns an `Effect<SignerIdentity, SignatureKitError>` with the holder's essential fields. This is the cheap read: subject, issuer, serial number, thumbprint, validity window, and the optional `document`. Provide a `Signatures` layer (here `a1SignaturesLayer`) so the accessor can resolve the loaded material.
  ...
  <Step title="Inspect the certificate">
    ...
    <Callout type="error" title="Caution">
      the accessor is `signatures.certificate()` — the full certificate read on the
      same `Signatures` service as `inspect()`. The
      `privateKeyPem` field is `Redacted<string>`: it does not appear in logs, in `console.log`,
      nor in `JSON.stringify`. To obtain the PEM you must explicitly call `Redacted.value(...)`,
      and only at the boundary where the key is actually consumed.
    </Callout>
  ...
  <Step title="Validity and ICP-Brasil">
    ...
    <Callout type="info">
      ...
      `2.16.76.1.3.1` identifies the e-CPF (the document is the holder's CPF) and `2.16.76.1.3.3`
      identifies the e-CNPJ (the document is the CNPJ). Extracting these fields is done by
      `extractBrazilianFields` internally and already appears ready in `identity.document` and in
      `cert.brazilian`.
    </Callout>
  </Step>
</Steps>
...
Every read failure is a typed `SignatureKitError` in the error channel. The most common ones when reading certificates:

- [`signature-kit.NO_CERTIFICATE`](/docs/errors#error-catalog) — the loaded material does not contain a certificate.
- [`signature-kit.X509_PARSE_FAILED`](/docs/errors#error-catalog) — the DER could not be parsed by `parseX509`.
- [`signature-kit.PEM_EXTRACTION_FAILED`](/docs/errors#error-catalog) — failed to extract the PEM from the certificate.

## See also

<Cards>
  <Card title="The signing boundary" href="/docs/signers">The SignerAdapter that loads the A1 and exposes the signing material.</Card>
  <Card title="Typed error channel" href="/docs/errors">NO_CERTIFICATE, X509_PARSE_FAILED and the other SignatureKitError.</Card>
</Cards>
