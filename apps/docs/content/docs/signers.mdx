...
description: "The SignerAdapter is the capability seam: it decides where the power to sign comes from without touching the document, is provided as the Signatures service, and is swappable without rewriting formats."
---

**The core idea:** the `SignerAdapter` is the single seam through which the power to sign enters. It answers _where_ the key comes from — an A1 `.pfx`, an HSM, a remote service — and always exposes the same shape. Everything above it (core, XML, PDF) talks to that shape, not to the backend.

```text
                    ┌──────────────────────────────┐
   core / XML / PDF │   talk to the same shape      │
                    └──────────────┬───────────────┘
                                   │
                          ┌────────▼────────┐
                          │  SignerAdapter  │  ← the capability seam
                          └────────┬────────┘
                                   │
              ┌────────────────────┼────────────────────┐
              │                    │                     │
...
The type below is the literal definition exported by `@signature-kit/core/config`. The doc-comment pins down the rule that the rest of this page details: the signer owns where the signing power comes from, and **never** owns format mutation.
...
- `id: string` — identity of the signing backend. A1 uses `"a1"`. Remote signers such as DocuSign, Clicksign, Assinafy and ZapSign use `create*SignatureRequest(...)` functions and do not go through this XML/PDF mutation seam.
...
The `@signature-kit/a1` package implements the seam from a `.pfx`/`.p12` file. `loadA1SignerAdapter(options)` does the asynchronous parsing and returns a ready `SignerAdapter`, with `id` equal to `"a1"`.
...
The core defines `Signatures` as a `Context.Service` with tag `"@signature-kit/core/Signatures"`. You never implement that service by hand: you pass a `SignerAdapter` to `signaturesLayer(signer)` and get a `Layer<Signatures>` back (via `Layer.succeed`). The accessors `signatures.{inspect, certificate, importSigningKey, sign, verify}` declare the dependency on `Signatures` and satisfy it with the provided layer.
...
For formats, A1 offers a shortcut: `a1SignaturesLayer(options)` returns a `Layer<Signatures, SignatureKitError>` directly, without you instantiating the adapter beforehand. `signXml` and `signPdf` require `Signatures` in their requirements channel; you just hand over that layer via `Effect.provide`.
...
Because the seam is a fixed-shape port, swapping where the signing power comes from is swapping the argument of `signaturesLayer(...)`. The signing work is identical in both cases; nothing about the format changes.
...
The doc-comment's rule is deliberate: the `SignerAdapter` owns **where** the signing power comes from, and **never** document mutation. It operates over `content: Uint8Array` and returns a `signature: Uint8Array` — opaque bytes. Embedding a signature into an enveloped XML or a PAdES PDF is the job of the format modules (`@signature-kit/xml`, `@signature-kit/pdf`), which consume the seam through `Effect.provide` and only because of that stay backend-neutral.

That cut is what keeps the application stable: the XML doesn't know whether the key came from a `.pfx` or an HSM, and the signer doesn't know whether the bytes it received are an NF-e or a PDF. Each side knows only its own half of the boundary.
...
<Cards>
  <Card title="signature-kit.WRONG_PASSWORD" href="/docs/errors#error-catalog">Incorrect certificate password in loadA1SignerAdapter.</Card>
  <Card title="signature-kit.NO_PRIVATE_KEY" href="/docs/errors#error-catalog">The file contains no private key, so importSigningKey and sign have nothing to sign with.</Card>
  <Card title="signature-kit.UNSUPPORTED_ALGORITHM" href="/docs/errors#error-catalog">Algorithm outside "rsa-sha256" / "rsa-sha512".</Card>
  <Card title="signature-kit.KEY_IMPORT_FAILED" href="/docs/errors#error-catalog">The CryptoKey could not be materialized in importSigningKey.</Card>
  <Card title="signature-kit.SIGN_FAILED" href="/docs/errors#error-catalog">The sign operation failed in the backend.</Card>
</Cards>
