...
**The core idea:** the `SignerAdapter` is the seam through which signing power enters. It answers _where_ the key comes from — an A1 `.pfx`, an HSM, a remote service — while everything above it (core, XML, PDF) talks to one fixed shape, not the backend.
...
- `id: string` — identity of the signing backend. A1 uses `"a1"`. Remote signers such as Clicksign, Assinafy, ZapSign, DocuSeal, and Documenso use `create*SignatureRequest(...)` functions and do not go through this XML/PDF mutation seam.
