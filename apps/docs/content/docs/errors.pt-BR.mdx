...
Toda falha no core, no signer A1 e nos formatos é um `SignatureKitError` no canal de erros do `Effect` — não uma exceção lançada com `throw`. O tipo de retorno carrega a falha: `Effect<SignatureArtifact, SignatureKitError>`. Isso torna o tratamento exaustivo e verificável pelo compilador: você discrimina pelo `code`, e tudo o que você não tratar continua se propagando, tipado.
...
Quando você só quer interceptar uma falha específica e deixar o restante se propagar, use `Effect.catchIf` com um predicado sobre `_tag` e `code`:
...
Além do `SignatureKitError`, os módulos de formato e os signers remotos adicionam suas próprias famílias tipadas ao canal de erros. Cada uma é uma tag distinta, então `Effect.catchTag` e `Effect.catchTags` separam as falhas por origem:
...
<Cards>
  ...
  <Card title="Signers remotos">Sem família paralela. Entrada inválida, HTTP remoto, formato de resposta e operação não suportada falham como `SignatureKitError` com `provider`, `operation`, `schemaName` e `status` quando esses metadados existem.</Card>
</Cards>
...
Nos signers remotos, as falhas de entrada, de HTTP e de formato de resposta chegam como `SignatureKitError`. Discrimine pelo `error.code` da mesma forma:
