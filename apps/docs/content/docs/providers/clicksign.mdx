...
- One document per request; the props schema is a single-document tuple, so passing more than one document fails at schema decode time as `signature-kit.INVALID_INPUT`.
