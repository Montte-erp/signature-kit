...
<Steps>
  <Step title="Instale os pacotes">
    Comece pelo runtime `@signature-kit/signatures` e pelo signer `@signature-kit/a1`.
  ...
  <Step title="Assine e verifique bytes">
    O serviço expõe `signatures.inspect`, `signatures.sign` e `signatures.verify`. O algoritmo é `"rsa-sha256"` ou `"rsa-sha512"`; o catálogo também aceita o legado `"rsa-sha1"`, suportado por compatibilidade, mas desencorajado.
