...
<Steps>
  <Step title="Install the packages">
    Start with the `@signature-kit/signatures` runtime and the `@signature-kit/a1` signer.
  ...
  <Step title="Sign and verify bytes">
    The service exposes `signatures.inspect`, `signatures.sign`, and `signatures.verify`. The algorithm is `"rsa-sha256"` or `"rsa-sha512"`; the catalog also accepts the legacy `"rsa-sha1"`, supported for compatibility but discouraged.
